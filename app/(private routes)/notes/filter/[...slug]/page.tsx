...
export async function generateMetadata({ params }: NotesPageProps): Promise<Metadata> {
  ...
  const title = tag === 'All' ? 'All my notes' : `My ${tag.toLocaleLowerCase()} notes`;
...
const NotesPage = async ({ params }: NotesPageProps) => {
  ...
  const tag = (await params).slug?.[0];
  const allowedTag = Object.values(NoteTag).includes(tag as NoteTag) ? (tag as NoteTag) : undefined;
